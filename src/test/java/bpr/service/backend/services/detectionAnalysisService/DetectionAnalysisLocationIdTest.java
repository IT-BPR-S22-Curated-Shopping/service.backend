...
class DetectionAnalysisServiceLocationIdTest {
    ...
    private void setLocationIdMock() {
        Mockito.when(detectionRepository
                .findDetectionSnapshotEntitiesByLocationIdAndTimestampBetween(
                        locationId,
                        from,
                        to
    ...
    @Test void emptySnapshotLocationAnalysis() {
        ...
        // Act
        try {
            analysis = detectionAnalysisService.locationAnalysis(locationId, from, to);
        } catch (NotFoundException e) {
            assertNotNull(e);
            assertEquals(String.format("No detections for location id %s in the given timeframe.", locationId), e.getMessage());
        }
        assertNull(analysis);
    }
